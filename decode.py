...
class Decoder:
    ...
    def decode(self):
        if (self.layer_type == CHROMINANCE
            ...
            raise ValueError('The length of DC chrominance {}or AC chrominance {} cannot be divided by 2 evenly to seperate into Cb and Cr.'.format(len(self.dc),len(self.ac)))

        shaped = {}
        if len(self.dc) != len(self.ac):
            raise ValueError('DC size {} is not equal to AC size {}.'.format(len(self.dc),len(self.ac)))
...
def decode_huffman(bit_seq, dc_ac, layer_type):
    ...
    while current_idx < len(bit_seq):
        ...
        while current_slice:
            ...
            raise KeyError(
                'Cannot find any prefix of {} in Huffman table.'.format(err_cache)
            )
